 ...
 * under the License.
 */
package io.ballerina.stdlib.mssql.compiler.analyzer;
...
public class RecordAnalyzer implements AnalysisTask<SyntaxNodeAnalysisContext> {
    ...
    public void perform(SyntaxNodeAnalysisContext ctx) {
        ...
        if (varSymOptional.isPresent()) {
            ...
            if (isMSSQLOptionsRecord(typeSymbol, Constants.Options.NAME)) {
    ...
    private boolean isMSSQLOptionsRecord(TypeSymbol type, String recordName) {
        if (type.typeKind() == TypeDescKind.UNION) {
            return ((UnionTypeSymbol) type).memberTypeDescriptors().stream()
                    .filter(typeDescriptor -> typeDescriptor instanceof TypeReferenceTypeSymbol)
                    .map(typeReferenceTypeSymbol -> (TypeReferenceTypeSymbol) typeReferenceTypeSymbol)
                    .anyMatch(typeReferenceTypeSymbol -> isMSSQLOptionsRecord(typeReferenceTypeSymbol, recordName));
        }
        if (type.typeKind() == TypeDescKind.TYPE_REFERENCE) {
            return isMSSQLOptionsRecord((TypeReferenceTypeSymbol) type, recordName);
        }
        return false;
    }

    private boolean isMSSQLOptionsRecord(TypeReferenceTypeSymbol typeSymbol, String recordName) {
        if (typeSymbol.typeDescriptor().typeKind() == TypeDescKind.RECORD) {
            ModuleSymbol moduleSymbol = typeSymbol.getModule().get();
            return MSSQL.equals(moduleSymbol.getName().get()) && BALLERINAX.equals(moduleSymbol.id().orgName())
